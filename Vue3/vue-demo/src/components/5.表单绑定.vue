...
<template>
    ...
    <div style="display: flex;">
        <div style="border: 1px solid black;width: 300px">
            <form>
                <h1>表单绑定:v-model</h1>
                <p style="background-color: black"><label>姓名(文本框)：</label><input v-model="data.userName" /></p>
                <p style="background-color: black"><label>同意协议(checkbox)：</label>
                ...
                <p style="background-color: black">
                    <label>性别(单选框)：</label>
                    <label><input type="radio" name="sex" v-model="data.gender" value="男">男</label>
                    <label><input type="radio" name="sex" v-model="data.gender" value="女">女</label>
                </p>
                <p style="background-color: black">
        ...
        <div style="border: 1px solid blue;width: 200px">
            <h1>结果预览</h1>
            <p style="background-color: black"><label>姓名：{{ data.userName }}</label></p>
            <p style="background-color: black"><label>同意协议：{{ data.agree }}</label>
            </p>
            <p style="background-color: black">
                <label>兴趣：</label>
                <li v-for="hobby in data.hobby">{{ hobby }}</li>
            </p>
            <p style="background-color: black">
            ...
            <p style="background-color: black">
                <label>课程：</label>
                {{ data.course }}
                <li v-for="course in data.course">{{ course }}</li>
            </p>
        </div>
    </div>
</template>
